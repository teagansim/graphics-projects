...
Animate( )
{
	...
	ms %= MS_PER_CYCLE;
	Time = (float)ms / (float)MS_PER_CYCLE;		// [0.,1.)	// makes the value of Time between 0. and slightly less than 1.
...
Display( )
{
	...
	float xlight = 4 * cos(12.566 * Time);
	float zlight = 4 * sin(12.566 * Time);


	if (SpotLight == true)
	{
		SetPointLight(GL_LIGHT0, xlight, 6., zlight, RedLight, GreenLight, BlueLight);
	}
	else
	{
		SetSpotLight(GL_LIGHT0, xlight, 6., zlight, 0., -1., 0., RedLight, GreenLight, BlueLight);
	}
	...
	glPushMatrix();
		glTranslatef(0.f, 1.25f, -3.f);
		glRotatef(90., 1., 0., 0.);
		SetMaterial(0.f, 0.f, 1.f, 300.f);
		glCallList(TorusDL);
	glPopMatrix();
